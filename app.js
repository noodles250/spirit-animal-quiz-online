...
function loadQuestion() {
    ...
    if (elements.progressText) {
        elements.progressText.textContent = `Question ${AppState.currentQuestion + 1} of ${AppState.totalQuestions}`;
    }
    ...
    if (elements.answersGrid) {
        elements.answersGrid.innerHTML = question.options.map((option, index) => `
...
function finishQuiz() {
    // Calculate spirit animal result
    const spiritAnimal = calculateSpiritAnimal();
    AppState.result = spiritAnimal;
    
    // Store result for email collection page
    localStorage.setItem('spiritAnimal', spiritAnimal);
    localStorage.setItem('quizCompleted', 'true');
    localStorage.setItem('completionTime', new Date().toISOString());
    
    // Redirect to email collection page instead of showing result page
    window.location.href = `email-collection.html?animal=${spiritAnimal}`;
}
...
function calculateSpiritAnimal() {
    ...
    Object.keys(SpiritAnimals).forEach(animal => {
        scores[animal] = 0;
    });
    ...
    AppState.answers.forEach(answer => {
        Object.entries(answer.animals).forEach(([animal, points]) => {
            if (scores[animal] !== undefined) {
                scores[animal] += points * answer.weight;
            }
        });
    });
    
    // Find the animal with highest score
    const topAnimal = Object.entries(scores).reduce((max, current) => {
